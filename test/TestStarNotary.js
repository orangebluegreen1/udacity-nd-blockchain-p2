...
// Extra unit test not mentioned in the rubric
it('prevents a user from transferring a star they do not own', async() => {
    let instance = await StarNotary.deployed()
    let starId = 10
    let user = accounts[1]
    let incorrectUser = accounts[2]
    await instance.createStar('awesome star', starId, {from: user})

    await assert.rejects(() => instance.transferStar(incorrectUser, starId))
})
...
it('lookUptokenIdToStarInfo test', async() => {
    ...
    let starId = 11
    let user = accounts[0]
    const starName = 'awesome star name'
